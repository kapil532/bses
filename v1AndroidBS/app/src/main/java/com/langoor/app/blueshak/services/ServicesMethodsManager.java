...
public class ServicesMethodsManager {
    ...
    private void parseResponse(Context context, Object obj, JSONObject resp) {
       ...
       /* }else if (obj instanceof FacebookRegisterModel){
            if (resp.has("error")) {
                ErrorModel errorModel = new ErrorModel();
                if (errorModel.toObject(resp.toString())){
                    mUiCallBack.OnSuccessFromServer(errorModel);
                ...
                if (facebookRegisterModel.toObject(resp.toString())) {
                    mUiCallBack.OnSuccessFromServer(facebookRegisterModel);
                } else {
                    mUiCallBack.OnError(context.getString(R.string.ErrorResponseData));
                }
            }
    ...
    public void getSellerProducts(Context context, String seller_id,ServerResponseInterface mCallInterface, String TAG) {
        setCallbacks(mCallInterface);
        String param = "token=" + GlobalFunctions.getSharedPreferenceString(context, GlobalVariables.SHARED_PREFERENCE_TOKEN);
    ...
    public void getMySalesList(Context context, ServerResponseInterface mCallInterface, String TAG) {
        setCallbacks(mCallInterface);
        String param = "token=" + GlobalFunctions.getSharedPreferenceString(context, GlobalVariables.SHARED_PREFERENCE_TOKEN);
        getData(context, new SalesListModel(), ServerConstants.URL_getListMySales, param, TAG);
    }
    ...
    public void markInappropirate(Context context, String productID, String content, ServerResponseInterface mCallInterface, String TAG) {
        setCallbacks(mCallInterface);
        String param = "token=" + GlobalFunctions.getSharedPreferenceString(context, GlobalVariables.SHARED_PREFERENCE_TOKEN);
        param += "&product_id=" + productID;
        param += "&content=\"" + content + "\"";
        postData(context, new StatusModel(), ServerConstants.URL_markInappropirate, param, TAG);
    }
...
/*    public void createSale(Context context, CreateSalesModel salesModel, ServerResponseInterface mCallInterface, String TAG) {
            ...
            ArrayList<CreateImageModel> images = salesModel.getImages();
                if(images.size()>0){
                    for (int j = 0; j < images.size(); j++) {
                        ...
                        if(base64.length()>0){
                            System.out.println("######base64###indide#########"+base64.length());
                            System.out.println("######base64###indide#########"+base64);
                            CreateImageModel model = new CreateImageModel();
                            model.setImage(base64);
                            if (j == 0) {
                                model.setDisplay(true);
                            ...
                            System.out.println("#####images.get(j)#####"+base64.length());
                            /*images.remove(j);*/
    ...
    public void updateItemStatus(Context context, String product_id, boolean isAvailable, ServerResponseInterface mCallInterface, String TAG) {
        setCallbacks(mCallInterface);
        String param = "token=" + GlobalFunctions.getSharedPreferenceString(context, GlobalVariables.SHARED_PREFERENCE_TOKEN);
        ...
        param += "&status=" + status;
        postData(context, new StatusModel(), ServerConstants.URL_MARK_ITEM_SOLD, param, TAG);
    }
    ...
    public void getItemInfo(Context context, String product_id, ServerResponseInterface mCallInterface, String TAG) {
        setCallbacks(mCallInterface);
        String param = "product_id=" + product_id;
...
*/
    ...
    public void ask_for_phone_number(Context context,String email, ServerResponseInterface mCallInterface, String TAG) {
        setCallbacks(mCallInterface);
        String param ="email="+email;
        postData(context, new FacebookRegisterModel(), ServerConstants.URL_UserFb_ask_phoneRegistration, param, TAG);
